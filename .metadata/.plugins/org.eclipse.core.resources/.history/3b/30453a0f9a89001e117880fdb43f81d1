...
import br.com.healthsolu.service.PercentualGorduraService;
...
@Path("/percentualGordura") //http://localhost:8080/HealthSolu/api/percentualGordura
public class PercentualGorduraResource {
	
	private PercentualGorduraService service;
	
	public PercentualGorduraResource() throws ClassNotFoundException, SQLException {
	...
	public Response cadastrar(PercentualGordura percentualGordura, @Context UriInfo uri) throws ClassNotFoundException, SQLException, IdNotFoundException, GenderNotFoundException {
		try {
			service.cadastrar(percentualGordura);
			UriBuilder uriBuilder = uri.getAbsolutePathBuilder();
			uriBuilder.path(String.valueOf(percentualGordura.getId()));
